...
// TestDomainsService_TenantFindingStats verifies tenant-wide critical/warning
// domain counts and that they do not leak across tenants.
func TestDomainsService_TenantFindingStats(t *testing.T) {
	ctx := context.Background()
	pc, err := testhelpers.CreatePostgresContainer(ctx)
	if err != nil {
		t.Fatalf("create container: %v", err)
	...
	t1 := createTenant(t, ctx, pc, "owner1@example.com")
	p1 := ownerPrincipal(t1)

	dCrit := seedDomain(t, ctx, pc, t1, "crit.example.com")
	dWarn := seedDomain(t, ctx, pc, t1, "warn.example.com")
	seedDomain(t, ctx, pc, t1, "healthy.example.com")
	seedSPFFinding(
		t,
		ctx,
		pc,
	...
	crit, warn, err := ds.TenantFindingStats(ctx, p1)
	if err != nil {
		t.Fatalf("TenantFindingStats: %v", err)
	}
	if crit != 1 {
	...
	// A second tenant's critical finding must not leak into tenant 1's stats.
	t2 := createTenant(t, ctx, pc, "owner2@example.com")
	dOther := seedDomain(t, ctx, pc, t2, "other.example.com")
	seedSPFFinding(
		t,
		ctx,
		pc,
	...
	crit2, _, err := ds.TenantFindingStats(ctx, p1)
	if err != nil {
		t.Fatalf("TenantFindingStats (after t2): %v", err)
	}
	if crit2 != 1 {
		t.Errorf("tenant 1 critical leaked: = %d, want 1", crit2)
	}
}
